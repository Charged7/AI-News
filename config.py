...
# Параметри OpenAI для персонального відбору та batch-підсумків новин.
...
OPENAI_RELEVANCE_BATCH_SIZE = _env_int("OPENAI_RELEVANCE_BATCH_SIZE", 6)
OPENAI_RELEVANCE_MAX_TOKENS = _env_int("OPENAI_RELEVANCE_MAX_TOKENS", 5000)
OPENAI_RELEVANCE_RETRY_MISSING_LIMIT = _env_int(
    "OPENAI_RELEVANCE_RETRY_MISSING_LIMIT", 0
)

# Профіль можна передати inline, але файл зручніший для довгого структурованого prompt.
NEWS_PREFERENCES_PATH = os.getenv("NEWS_PREFERENCES_PATH") or "data/user_preferences.md"
USER_NEWS_PREFERENCES = os.getenv("USER_NEWS_PREFERENCES", "").strip()

# Межі вибірки та збереження історії відправок.
NEWS_LOOKBACK_HOURS = _env_int("NEWS_LOOKBACK_HOURS", 24)
NEWS_MIN_RELEVANCE_SCORE = _env_int("NEWS_MIN_RELEVANCE_SCORE", 70)
